...
class DebateSystem:
    ...
    def __init__(self, num_agents: int = 5, question_start_index: int = 1, round0_answers: dict = None, round0_reasoning: dict = None, round0_peer_answers: dict = None):
        """__init__ helper."""
        ...
        self.round0_peer_answers = round0_peer_answers if round0_peer_answers else {}
    ...
    def set_round0_peer_answers(self, round0_peer_answers: dict):
        self.round0_peer_answers = round0_peer_answers
    ...
    def run_debate(self, question: str, num_rounds: int = 3, question_index: Optional[int] = None, question_id: str = None, correct_answer: Optional[str] = None, signal_type: SignalType = SignalType.REASONING, debate_round: int = None, max_retries_per_agent: int = 2) -> Dict:
        ...
        Signal types:
        - SELF_REFLECTION: Agents only see the question (no previous answers, no other agents' info)
        ...
        if run_round_1 and num_rounds >= 1:
            ...
            for agent in self.agents:
                ...
                peer_answers = (
                    self.round0_peer_answers
                    .get(str(question_id), {})
                    .get(agent.agent_id)
                )
                if peer_answers:
                    peer_agent_ids = [
                        peer_id for peer_id in range(1, self.num_agents + 1)
                        if peer_id != agent.agent_id
                    ]
                    other_opinions = []
                    for idx, peer_answer in enumerate(peer_answers[:len(peer_agent_ids)]):
                        peer_agent_id = peer_agent_ids[idx]
                        other_opinions.append({
                            "agent_id": peer_agent_id,
                            "opinion": peer_answer,
                            "reasoning": self.round0_reasoning.get(str(question_id), {}).get(peer_agent_id, "")
                        })
                else:
